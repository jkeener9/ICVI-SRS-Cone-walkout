...
d = "\\\\10.16.72.207\\public\\justin\\__JustinPythonCode__\\20180425_7p5mm\\"
...
for k in glob.glob(d + '*.dcm'):
    ...
    for i in range(np.asscalar(maxpixloc[1])-round(0.4*ConeDiapix),np.asscalar(maxpixloc[1])+round(0.4*ConeDiapix)):
        spline=UnivariateSpline(y,dcmFilespixNormROI[:,i]-np.max(dcmFilespixNormROI[:,i])/2,s=0)  #see sbcrowe.net/python-exercise-field-size-calculator/
        r1,r2=spline.roots()
        fwhm=abs(r1-r2)
        if fwhm > maxfwhmy:
            maxfwhmy=fwhm
    ...
    for j in range(np.asscalar(maxpixloc[0])-round(0.4*ConeDiapix),np.asscalar(maxpixloc[0])+round(0.4*ConeDiapix)):
        spline=UnivariateSpline(x,dcmFilespixNormROI[j,:]-np.max(dcmFilespixNormROI[j,:])/2,s=0)  
        r1,r2=spline.roots()
        fwhm=abs(r1-r2)
        if fwhm > maxfwhmx:
            maxfwhmx=fwhm
