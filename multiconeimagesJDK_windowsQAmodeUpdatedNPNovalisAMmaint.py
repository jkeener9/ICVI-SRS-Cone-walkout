...
d = "\\\\ceinvfs03s\\shared\\Centura Physics\\Physics Team\\Justin\\__JustinPythonCode__\\20201219_6mm_Novalis\\"  #Enter Directory where dicom image files are saved
pp = PdfPages(d + "ConeAnalysisResults.pdf")
ConeDiamm=6  #Enter Cone Diameter in mm
BeamCenters=[] 
...
for k in glob.glob(d + '*.dcm'):
    ...
    minpix=np.min(dcmFilespix)
    dcmFilespixNorm=(dcmFilespix-minpix)/(maxpix-minpix)
    ...
    x=np.linspace(left,left+WidthROI-1,WidthROI)
